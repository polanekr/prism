...
def draw_prism_flowchart():
    ...
    def draw_box(x, y, w, h, text, color, title=None):
        ...
        ax.add_patch(box)
        if title:
            ax.text(x+w/2, y+h-0.3, title, ha='center', va='top', fontsize=11, fontweight='bold')
            ax.text(x+w/2, y+h/2-0.2, text, ha='center', va='center', fontsize=10, linespacing=1.4)
        else:
            ax.text(x+w/2, y+h/2, text, ha='center', va='center', fontsize=11, fontweight='bold', linespacing=1.4)
    ...
    def draw_arrow(start, end, label=""):
        ...
        if label:
            ...
            ax.text(mx, my+0.2, label, ha='center', va='bottom', fontsize=9, fontstyle='italic', color='#616161')
    ...
    ax.text(1, 8, "1. Experimental Setup & Irradiation", fontsize=14, fontweight='bold')
    ...
    ax.text(1, 5.2, "2. Film Dosimetry & Calibration", fontsize=14, fontweight='bold')
    ...
    draw_arrow(out_film_bot, in_2dmap_top, "TIFF files")
    draw_arrow(out_calib, in_2dmap, "Calibration curve")
    
    # --- PHASE 3: 3D PRISM Reconstruction ---
    ax.text(1, 2.7, "3. PRISM 3D Reconstruction", fontsize=14, fontweight='bold')
    ...
    draw_arrow((12.5, y_bot+0.6), (11.5, y_bot+0.6), "Sparse 2D PDD")
    
    draw_arrow(in_gp, out_het)
    
    # --- PHASE 4: Biological Modeling ---
    ax.text(1, 0, "4. Radiobiological Output", fontsize=14, fontweight='bold')
    _, _, _, in_bio_top = draw_box(0, y_bot, 3, 1.2, "Biological Dosimetry\n(dDVH & EUD with \nBayesian Uncertainty)", c_bio)
    
    draw_arrow(in_het, (3, y_bot+0.6), "Corrected 3D Volume")
    ...
    plt.ylim(-0.5, 8.5)
    plt.tight_layout()
    plt.savefig("fig_framework_flowchart.png", dpi=300)
    plt.show()
